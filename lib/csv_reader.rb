...
class CSVReader
  ...
  def initialize(filename)          # automatically run when program starts
  ...
  def read                          
    ...
    while (!f.eof? && next_line = f.readline)   # I understand the while it is not the end of the file, but where does "&& next_line = readline" come in, that doesn't seem to be a condition???
      values = next_line.split(',')             # values is now an array, because we are splitting the CSV by comma.  is this called "value" because it's a hash and is the value part of key/value pair???
      hash = create_hash(values)                # in this hash, 
